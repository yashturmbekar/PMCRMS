...
const OfficerManagementPage: React.FC = () => {
  ...
  const handleInviteOfficer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        setSuccessMessage(
          "Officer invited successfully! Invitation link sent to email."
        );
  ...
  const handleEditOfficer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await adminService.updateOfficer(selectedOfficer.id, {
        ...
        // Don't send role - we're not changing it in this modal
      ...
      if (response.success) {
        setSuccessMessage(
          editForm.email !== selectedOfficer.email
            ? "Officer updated successfully! Invitation link has been sent to the new email address."
    ...
    <>
      {submitting && (
        <FullScreenLoader
          ...
          submessage="Please wait while we send the invitation email"
        />
      )}
      {updating && (
        <FullScreenLoader
              ...
              <form onSubmit={handleEditOfficer} style={{ padding: "24px" }}>
                ...
                <div style={{ marginBottom: "24px" }}>
                  ...
                  {editForm.email !== selectedOfficer?.email && (
                    ...
                    >
                      ⚠️ Changing email will send a new invitation link
                    </p>
                  )}
                </div>
                ...
                {editForm.email !== selectedOfficer?.email && (
                  ...
                  >
                    <p
                      style={{ color: "#92400E", fontSize: "13px", margin: 0 }}
                    >
                      ⚠️ An invitation link will be sent to the updated email
                      address. Officer must complete password setup to
                      reactivate account.
                    </p>
                  </div>
                )}
