    ...
    public class PositionRegistrationController : ControllerBase
    {
            ...
            [FromBody] PositionRegistrationRequestDTO request)
        {
            try
            {
                ...
                // Update documents - UPDATE existing documents instead of removing and re-adding
                // IMPORTANT: Only update user-submitted documents, preserve system-generated ones
                ...
                var existingUserDocuments = application.Documents
                    .Where(d => !systemGeneratedDocTypes.Contains(d.DocumentType))
                    .ToList();
                
                _logger.LogInformation("[Resubmit] Found {Count} existing user documents", existingUserDocuments.Count);
                
                // Track which documents to keep, update, or add
                var documentsToRemove = new List<SEDocument>();
                
                foreach (var doc in request.Documents)
                {
                    // Try to find existing document by FileId
                    var existingDoc = existingUserDocuments.FirstOrDefault(d => d.FileId == doc.FileId);
                    
                    if (existingDoc != null)
                    {
                        // UPDATE existing document
                        _logger.LogInformation("[Resubmit] Updating existing document {FileId}", doc.FileId);
                        
                        existingDoc.DocumentType = doc.DocumentType;
                        existingDoc.FileName = doc.FileName;
                        existingDoc.DocumentName = doc.DocumentName; // Update custom document name
                        ...
                        // Only update file content if new base64 data is provided
                        if (!string.IsNullOrEmpty(doc.FileBase64))
                        {
                            try
                            {
                                existingDoc.FileContent = Convert.FromBase64String(doc.FileBase64);
                                _logger.LogInformation("[Resubmit] Updated file content for document {FileId}", doc.FileId);
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, "[Resubmit] Error converting base64 for document {FileId}", doc.FileId);
                                return BadRequest(new { error = $"Invalid file data for document {doc.FileName}" });
                            }
                        }
                        
                        // Mark this document as processed
                        existingUserDocuments.Remove(existingDoc);
                    }
                    else
                    {
                        // ADD new document (didn't exist before)
                        _logger.LogInformation("[Resubmit] Adding new document {FileId}", doc.FileId);
                        ...
                        if (!string.IsNullOrEmpty(doc.FileBase64))
                        {
                            ...
                            {
                                _logger.LogError(ex, "[Resubmit] Error converting base64 for new document {FileId}", doc.FileId);
                                return BadRequest(new { error = $"Invalid file data for document {doc.FileName}" });
                            }
                        }
                ...
                // Remove documents that were in the DB but not in the request (user deleted them)
                if (existingUserDocuments.Any())
                {
                    _logger.LogInformation("[Resubmit] Removing {Count} deleted documents", existingUserDocuments.Count);
                    _context.SEDocuments.RemoveRange(existingUserDocuments);
                }
